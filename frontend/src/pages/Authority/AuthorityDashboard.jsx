    ...
    <nav style={{ background:"#060e1f", borderBottom:"1px solid rgba(255,153,51,0.2)", position:"sticky", top:0, zIndex:100 }}>
      {showLogoutConfirm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:500 }}>
          <div style={{ background:"#0a1628", border:"1px solid rgba(255,153,51,0.4)", padding:"24px", width:"320px", textAlign:"center" }}>
            <div style={{ fontSize:14, fontWeight:700, marginBottom:10 }}>Confirm Logout</div>
            <div style={{ fontSize:11, color:"#94a3b8", marginBottom:18 }}>Are you sure you want to logout?</div>
            <div style={{ display:"flex", gap:10 }}>
              <button className="btn-ghost" style={{ flex:1 }} onClick={() => setShowLogoutConfirm(false)}>Cancel</button>
              <button className="btn-danger" style={{ flex:1 }} onClick={() => { localStorage.removeItem("token"); window.location.href = "/login"; }}>Logout</button>
            </div>
          </div>
        </div>
      )}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 24px", height:54 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <button onClick={() => setShowLogoutConfirm(true)} style={{
            ...
            padding:"6px 12px", cursor:"pointer", fontFamily:"'JetBrains Mono',monospace",
          }}>Logout</button>
        </div>
      </div>
    </nav>
...
function BidCard({ bid, rank, isAssigned, onAssign, canAssign, assigning }) {
  ...
  return (
    <div className="card" style={{ padding:16, position:"relative", borderColor: isAssigned ? "rgba(34,197,94,0.55)" : isTop ? "rgba(255,153,51,0.6)" : undefined }}>
      {isTop && !isAssigned && (
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:9, color:"#64748b", marginTop:1 }}>✅ {vol?.volunteerDetails?.totalTasksCompleted || 0} tasks completed</div>
        </div>
      </div>
      ...
      {canAssign && !isAssigned && (
        <button className="btn-primary" style={{ width:"100%", marginTop:4 }} disabled={assigning} onClick={() => onAssign(bid._id, bid.volunteer._id)}>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding:20 }}>
          ...
          <div className="section-label">Volunteer Applications — {bidsLoading ? "…" : `${bids.length} applied`}</div>
          {bidsLoading && <div style={{ textAlign:"center", color:"#475569", fontSize:11, padding:24 }}>Loading bids…</div>}
          {!bidsLoading && bids.length === 0 && (
            <div style={{ textAlign:"center", color:"#475569", fontSize:11, padding:24, border:"1px solid rgba(255,255,255,0.05)" }}>No volunteer applications yet.</div>
          )}
          {!bidsLoading && sorted.length > 0 && (
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              {sorted.map((bid, rank) => (
                <BidCard key={bid._id} bid={bid} rank={rank} isAssigned={isAssignedTo(bid)} onAssign={handleAssign} canAssign={complaint.status === "pending"} assigning={assigning} />
              ))}
            </div>
          )}
        </div>
...
function AssignTaskView({ complaints, setComplaints, loading }) {
  ...
  return (
    <>
      {modal && (
        <ComplaintModal complaint={modal} onClose={() => setModal(null)} onAssigned={handleAssigned} onResolved={handleResolved} />
      )}
...
function PendingView({ complaints, setComplaints, loading }) {
  ...
  const handleResolve = async (complaintId) => {
    ...
    try {
      const res = await fetch(`${API}/volunteer/complaint/${complaintId}/resolve`, { method:"PUT", headers: jsonHeaders() });
      const data = await res.json();
      if (!data.success) throw new Error(data.message);
      setComplaints(prev => prev.map(c => c._id === complaintId ? { ...c, status:"resolved", resolvedAt: new Date() } : c));
    } catch (e) { alert(e.message); }
  ...
  return (
    <div style={{ padding:24 }}>
      ...
      {active.length === 0 && (
        <div style={{ textAlign:"center", color:"#475569", padding:60, border:"1px solid rgba(255,255,255,0.05)", fontSize:11 }}>No active tasks right now.</div>
      )}
            ...
            <div key={c._id} className="card" style={{ overflow:"hidden" }}>
              <div style={{ position:"relative" }}>
                {c.photo
                  ...
                  : <div style={{ width:"100%", aspectRatio:"16/6", background:"#0d1b2e", display:"flex", alignItems:"center", justifyContent:"center", fontSize:36, borderBottom:"1px solid rgba(255,153,51,0.1)" }}>{categoryIcon(c.category)}</div>
                }
                ...
                <div style={{ position:"absolute", bottom:8, left:12, fontSize:8, color:"#FF9933", letterSpacing:"0.1em", fontWeight:700 }}>#{c._id?.slice(-6).toUpperCase()}</div>
              ...
              <div style={{ padding:18 }}>
                ...
                <div style={{ fontSize:9, color:"#64748b", marginBottom:6 }}>📍 {c.location} · {timeAgo(c.createdAt)}{c.postedBy?.name && ` · 👤 ${c.postedBy.name}`}</div>
                {c.description && (
                  <p style={{ fontSize:10, color:"#94a3b8", lineHeight:1.6, marginBottom:14, display:"-webkit-box", WebkitLineClamp:3, WebkitBoxOrient:"vertical", overflow:"hidden" }}>{c.description}</p>
                )}
                {vol && (
                  <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", padding:12, marginBottom:14 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:9, color:"#475569", marginTop:2 }}>✅ {vol.volunteerDetails?.totalTasksCompleted || 0} tasks completed</div>
                      </div>
                    </div>
                    {bid && (
                      <>
                        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:10 }}>
                          <div><div style={{ fontSize:8, color:"#475569" }}>Agreed Amount</div><div style={{ fontSize:15, fontWeight:900, color:"#60a5fa" }}>₹{bid.estimatedAmount?.toLocaleString() || "—"}</div></div>
                          <div><div style={{ fontSize:8, color:"#475569" }}>Est. Days</div><div style={{ fontSize:15, fontWeight:900, color:"#f59e0b" }}>{bid.estimatedDays || "—"}d</div></div>
                          <div><div style={{ fontSize:8, color:"#475569" }}>Assigned On</div><div style={{ fontSize:10, fontWeight:700, color:"#94a3b8", marginTop:4 }}>{c.updatedAt ? new Date(c.updatedAt).toLocaleDateString("en-IN", { day:"2-digit", month:"short" }) : "—"}</div></div>
                        </div>
                        <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", padding:"8px 10px", fontSize:9, color:"#64748b", lineHeight:1.7 }}>
                          🏦 {bid.bankDetails?.bankName} &nbsp;·&nbsp; A/C: ••••{bid.bankDetails?.accountNumber?.slice(-4)} &nbsp;·&nbsp; IFSC: {bid.bankDetails?.ifsc} &nbsp;·&nbsp; {bid.bankDetails?.accountHolder}
                        </div>
                      </>
                    )}
                  </div>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button className="btn-ghost" style={{ flex:1 }} onClick={() => vol?.phone && window.open(`tel:${vol.phone}`)}>📞 {vol?.phone || "No phone"}</button>
                  <button className="btn-primary" style={{ flex:1 }} disabled={resolving[c._id]} onClick={() => handleResolve(c._id)}>{resolving[c._id] ? "Resolving…" : "✅ Mark Resolved"}</button>
                </div>
              </div>
            </div>
          );
  ...
  return (
    <div style={{ padding:24 }}>
      ...
      <div style={{ marginBottom:16 }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by title, location, volunteer, reporter, or ID…" style={{ maxWidth:440 }} />
      </div>
      <div className="card" style={{ overflow:"hidden" }}>
              ...
              return (
                <tr key={r._id} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)", background: i%2===0 ? "transparent" : "rgba(255,255,255,0.01)", verticalAlign:"top" }}>
                  <td style={{ padding:"12px 14px" }}>
                    {r.photo ? <img src={r.photo} alt={r.title} style={{ width:72, height:54, objectFit:"cover", border:"1px solid rgba(255,153,51,0.2)", display:"block" }} />
                  ...
                  <td style={{ padding:"12px 14px", maxWidth:200 }}>
                    ...
                    <div style={{ display:"flex", gap:4, marginBottom:4, flexWrap:"wrap" }}><Badge label={categoryLabel(r.category)} className="text-blue-400 bg-blue-900/20 border-blue-700/40" /></div>
                    ...
                    {r.description && <div style={{ fontSize:9, color:"#475569", lineHeight:1.55, display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" }}>{r.description}</div>}
                  </td>
                  <td style={{ padding:"12px 14px", whiteSpace:"nowrap" }}>
                    {r.postedBy ? (<><div style={{ fontSize:10, fontWeight:700, color:"white" }}>{r.postedBy.name||"—"}</div><div style={{ fontSize:9, color:"#64748b" }}>{r.postedBy.email}</div></>) : <span style={{ fontSize:9, color:"#475569" }}>—</span>}
                  </td>
                  <td style={{ padding:"12px 14px" }}>
                    {vol ? (
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        {bid?.selfie ? <img src={bid.selfie} alt="selfie" style={{ width:30, height:30, objectFit:"cover", border:"1px solid rgba(255,153,51,0.3)", flexShrink:0 }} />
                        ...
                        <div>
                          ...
                          <div style={{ fontSize:9, color:"#64748b" }}>{vol.email}</div>
                          {vol.phone && <div style={{ fontSize:9, color:"#64748b" }}>📱 {vol.phone}</div>}
                          <div style={{ fontSize:9, color:"#475569", marginTop:2 }}>✅ {vol.volunteerDetails?.totalTasksCompleted||0} tasks done</div>
                        </div>
                      </div>
                  ...
                  <td style={{ padding:"12px 14px", whiteSpace:"nowrap" }}>
                    {bid && bid.estimatedAmount ? (
                      <>
                        <div style={{ fontSize:14, fontWeight:900, color:"#60a5fa", marginBottom:2 }}>₹{bid.estimatedAmount.toLocaleString()}</div>
                        <div style={{ fontSize:9, color:"#f59e0b", marginBottom:6 }}>{bid.estimatedDays}d estimated</div>
                        {bid.bankDetails?.bankName && (
                          <div style={{ fontSize:9, color:"#475569", lineHeight:1.65 }}>
                            🏦 {bid.bankDetails.bankName}<br />A/C: ••••{bid.bankDetails.accountNumber?.slice(-4)}<br />IFSC: {bid.bankDetails.ifsc}<br />{bid.bankDetails.accountHolder}
                          </div>
                        )}
                      </>
                  ...
                  <td style={{ padding:"12px 14px", whiteSpace:"nowrap" }}>
                    ...
                    <div style={{ fontSize:9, color:"#94a3b8", marginBottom:6 }}>{r.createdAt ? new Date(r.createdAt).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : "—"}</div>
                    <div style={{ fontSize:8, color:"#475569", marginBottom:2 }}>Resolved</div>
                    <div style={{ fontSize:9, color:"#22c55e", marginBottom:6 }}>{r.resolvedAt ? new Date(r.resolvedAt).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : "—"}</div>
                    <div style={{ fontSize:8, color:"#475569", marginBottom:2 }}>Duration</div>
                    <div style={{ fontSize:11, fontWeight:700, color:"#a78bfa" }}>{timeToResolve(r.createdAt, r.resolvedAt)}</div>
                  </td>
                </tr>
              );
            })}
      ...
      <div style={{ borderRight:"1px solid rgba(255,153,51,0.15)", display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ overflowY:"auto", flex:1 }}>
          {filtered.map(v => {
            ...
            return (
              <div key={v._id} className={`vol-row ${selected?._id===v._id?"active":""}`} onClick={()=>setSelected(v)} style={{ borderLeft: selected?._id===v._id ? "2px solid #FF9933" : "2px solid transparent" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:9, color:"#475569", marginTop:2 }}>✅ {v.volunteerDetails?.totalTasksCompleted||0} tasks</div>
                  </div>
          ...
          {filtered.length === 0 && <div style={{ padding:40, textAlign:"center", color:"#475569", fontSize:11 }}>No volunteers found.</div>}
        </div>
      </div>
          ...
          <>
            <div className="card" style={{ padding:20, marginBottom:16 }}>
              <div style={{ display:"flex", alignItems:"flex-start", gap:16 }}>
                ...
                <Badge label={selected.volunteerDetails?.isAvailable?"available":"busy"} className={selected.volunteerDetails?.isAvailable?"text-green-400 bg-green-900/20 border-green-700/40":"text-amber-400 bg-amber-900/20 border-amber-700/40"} />
              </div>
            </div>
