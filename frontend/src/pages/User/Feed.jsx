...
// Renders a small pinned map using coordinates from geoLocation field.
...
function ComplaintMap({ geoLocation, accuracy }) {
  ...
  useEffect(() => {
    if (!geoLocation?.coordinates || instanceRef.current) return;
    ...
    loadLeaflet().then((L) => {
      ...
      // Custom amber pin matching the portal style
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <div style={{
        ...
        left: 0,
        right: 0,
        background: "linear-gradient(to top, rgba(6,14,31,0.92), transparent)",
        ...
        <span style={{
          ...
          📍 {geoLocation.coordinates[1].toFixed(5)}, {geoLocation.coordinates[0].toFixed(5)}
        </span>
        {accuracy && (
          <span style={{
            fontSize: 9,
            fontFamily: "'JetBrains Mono', monospace",
            color: accuracy <= 50 ? "#34d399" : accuracy <= 200 ? "#60a5fa" : "#f59e0b",
            border: `1px solid ${accuracy <= 50 ? "rgba(52,211,153,0.4)" : accuracy <= 200 ? "rgba(96,165,250,0.4)" : "rgba(245,158,11,0.4)"}`,
            padding: "1px 6px",
          }}>
            ±{accuracy}m
          </span>
...
export default function Feed() {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/api/v1/volunteer/my-bids`, {
      ...
      .then(r => r.json())
      .then(d => {
        if (d.success) setAppliedComplaints(new Set(d.bids.map(b => b.complaint?.toString())));
      })
      .catch(console.error);
  }, [isVolunteer]);
  ...
  return (
    <div className="min-h-screen bg-[#060e1f] text-white flex">
      <style>{`
        ...
        /* Fix Leaflet z-index conflicts inside our dark portal */
        .leaflet-container { z-index: 0 !important; font-family: 'JetBrains Mono', monospace; }
        ...
        <div className="flex-1 gov-grid p-6 overflow-auto">
          ...
          {!loading && (
            <div className="max-w-4xl flex flex-col gap-5">
              ...
              {complaints.map((post, i) => (
                <div key={post._id || i} className="border border-white/10 hover:border-amber-700/40 bg-[#0a1628] transition group overflow-hidden">
                  ...
                  {/* ── Leaflet Map ── shown when geoLocation is available */}
                  <ComplaintMap
                    geoLocation={post.geoLocation}
                    accuracy={post.gpsAccuracyM}
                  />
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center gap-4 mt-3 text-[10px] font-mono-gov text-slate-500">
                      {/* Show human-readable address if available */}
                      <span>📍 {post.location || "Location unavailable"}</span>
                      <span>🕐 {timeAgo(post.createdAt)}</span>
                      {post.postedBy?.name && <span>👤 {post.postedBy.name}</span>}
                    </div>
